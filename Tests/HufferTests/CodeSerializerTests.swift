...
struct CodeSerializerTests {
    ...
    func createTestingClasses() -> (IOHandlerMock, CodeSerializer) {
        let handler = IOHandlerMock()
        let serializer = CodeSerializer(ioHandler: handler)
        return (handler, serializer)
    }

    @Test func getCodeWith16Bits() throws {
        let (handler, serializer) = createTestingClasses()
        let expectedOutput: [UInt8] = [ 0b11000011, 0b00111100]
        try serializer.writeContent("abba", codes)
    ...
    @Test func getCodeWith12Bits_LastByteFilledUpWithZeros() throws {
        ...
        try serializer.writeContent("abb", codes)
    ...
    @Test func getCodeWithMoreThan8Bits() throws {
        ...
        try serializer.writeContent("ab", longCodes)
